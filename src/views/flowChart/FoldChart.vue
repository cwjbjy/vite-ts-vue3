...
<template>
  <div id="myDiagramDiv" class="mygoChart"></div>
</template>
...
function dealShow() {
  ...
  var actionTemplate = $(
    go.Panel,
    'Horizontal',
    $(go.Shape, { width: 12, height: 12 }, new go.Binding('figure'), new go.Binding('fill')),
    $(go.TextBlock, { font: '10pt Verdana, sans-serif' }, new go.Binding('text')),
  );
    ...
    $(
      ...
      $(
        ...
        $(
          ...
          new go.Binding('visible', 'actions', function (acts: Record<string, any>[]) {
  ...
  myDiagram.linkTemplate = $(
    ...
    $(
      ...
      new go.Binding('visible', 'answer', function (a: number) {
        return a ? true : false;
      }),
    ),
  );
